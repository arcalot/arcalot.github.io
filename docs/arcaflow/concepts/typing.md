...
If the answer is no, then the typing system is here to help. Each plugin or workflow in Arcaflow has to explicitly state what data types it accepts for certain fields, and what their boundaries are. When a plugin then violates its own rules, the engine makes sure that corrupt data isn't used any further.
...
That's all it takes to render a nice form field or automatic documentation. You can read more about creating types in the [plugins section](../plugins/index.md) or the [workflows section](../workflows/index.md).
