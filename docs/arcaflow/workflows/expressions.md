...
| `bindConstants(list[any], any)`                               | list[object] |  Returns a list of objects with a new ID equal to schema name of the first parameter concatenated with the schema name of the second parameter. For each item in the list, an object with two properties is created. The item is assigned to this object's `item` property. The second argument of the `bindConstants` function is assigned to this object's `constant` property, thus repeating the second argument for each item in the list.                                                                                   |
...
In this case, we do not need to know the schema name of the type output generated by `bindConstants()`. When you do need to use this generated schema name [learn more about our workflow schema naming conventions.](schemas.md)