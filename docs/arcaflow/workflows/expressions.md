...
| `readFile(string)`                               | string | Given a string containing a file path, returns the contents of the indicated file as a UTF-8 character string. When passing a relative file path, the path must be relative to the working directory from which the Arcaflow process is executed. Additionally, shell environment variables are not expanded.                                                                                     |
